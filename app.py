...
def join_path_docs(docs):
    paths=[]
    for i in docs:
        path=i.metadata['file_path']
        file_path = path.split('/')[-1]
        file_path = "https://www.kaggle.com/datasets/bhavesh0124/human-rights?select=" + file_path
...
def query_llm_with_context_method(question, llm, prompt_template):
    ...
    context = vector_db.similarity_search(question, k=2)
    ...
    return llm_response['text'], paths
...
def query_llm_with_context():
    ...
    answer, paths = query_llm_with_context_method(question=question, llm=llm, prompt_template=prompt)
    #return jsonify({"response": answer, "paths": paths})
    return jsonify({
    "response": {
        "response": answer,
        "documentPaths": list(paths)  # Convert set to list
    }})
